...
You may recall from [cmos-transistors.md](../from-zero-to-one/cmos-transistors.md "mention") that CMOS circuits prefer NANDs and NORs over ANDs and ORs. But reading the equation by inspection from a multilevel circuit with NANDs and NORs can get pretty hairy. **Bubble pusing** is a helpful way to redraw these circuits so that the bubbles cancel out and the function can be more easily determined.
